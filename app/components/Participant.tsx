...
import { getPacketLoss$ } from '~/utils/rxjs/getPacketLoss$'  // Fix missing quote
...
type ParticipantProps = Props & JSX.IntrinsicElements['div']
...
export const Participant = forwardRef<HTMLDivElement, ParticipantProps>(
  ({ user, ...props }, ref) => {
    ...
    const {
      ...
      showDebugInfo,
      userMedia,
      room, // Use room object directly
    } = useRoomContext()
    const peerConnection = useObservableAsValue(partyTracks.peerConnection$)
    const id = user.id
    const isSelf = room.identity && id.startsWith(room.identity.id)
    ...
    // Simplificar totalmente a verificação de host - confiar no estado do usuário
    const isHost = Boolean(user.isHost)
    const canKickUsers = Boolean(room.identity?.isHost)

    console.log('Participant render:', {
      userName: user.name,
      rawIsHost: user.isHost,
      isHost,
      userId: user.id,
      canKickUsers,
      currentUserIsHost: room.identity?.isHost
    })
    ...
    return (
      <div
        {...props}
        className={cn(
          "grow shrink text-base basis-[calc(var(--flex-container-width)_-_var(--gap)_*_3)]",
          props.className
        )}
        ref={ref}
      >
        <Flipped flipId={id + pinned}>
          <div className={cn('h-full mx-auto overflow-hidden text-white opacity-0 animate-fadeIn',
            'relative max-w-[--participant-max-width] rounded-xl')}
          >
            {/* Sempre mostrar a coroa se isHost for true */}
            {isHost && (
              <div className="absolute top-2 left-2 z-10">
                <Tooltip content="Room Host">
                  <div className="indication-shadow">
                    <Icon 
                      ...
                      aria-hidden="true"
                    />
                    <VisuallyHidden>Room Host</VisuallyHidden>
                  </div>
                </Tooltip>
              </div>
            )}
            
            {/* Mostrar botão de kick se: 
                1. O usuário atual é host (canKickUsers)
                2. Não é o próprio usuário 
                3. Não é uma janela de compartilhamento
                4. Não é o AI */}
            {canKickUsers && !isSelf && !isScreenShare && !isAi && (
              <HoverFade className="absolute top-2 right-2 z-10">
                <Tooltip content="Kick User">
                  <button
                    className="p-2 rounded bg-red-500/80 hover:bg-red-600/80"
                    onClick={() => {
                  ...
                  >
                    <Icon type="userMinus" className="text-white" />
                    <VisuallyHidden>Kick User</VisuallyHidden>
                  </button>
                </Tooltip>
              </HoverFade>
            )}
